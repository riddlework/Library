...
private static boolean handleCommand() {
	...
	validateCommand(//pass list of strings);
...
private static void validateCommand(String[] input) {
	...
	String[] validCommands = {"addBook", "addBooks", "getBooks", "rate", "search", "setToRead", "suggestRead", "exit", "help"};
	
	for (int i=0; i<validCommands.length ; i++) {
		if (input[0].equals(validCommands[i])) { 
			if (input[0].equals("exit")) {return false;} 
			else {
				validateArgs(input);
				return true;
			}
...
private static void validateArgs(String[] input) {
	...
	switch (input[0]) {
		case "addBook":
			if (input.length == 3) {controller.addBook(input[1], input[2]);}
			break;
		case "addBooks":
			if (input.length == 2) {controller.addBooks(input[1]);}
			break;
		case "getBooks":
			if (input.length == 2) {controller.getBooks(input[1]);}
			break;
		case "rate":
			if (input.length == 3) {controller.rate(input[1], input[2]);}
			break;
		case "search":
			if (input.length == 3) {
				if (input[1].equals("rating") 
					&& (0 < Integer.valueOf(input[2])) 
					&& (6 > Integer.valueOf(input[2]))) {
					controller.searchRating(input[1], input[2]);
				}
				else if (input[1].equals("title")) {controller.searchTitle(input[1], input[2]);}
				else if (input[1].equals("author")) {controller.searchTitle(input[1], input[2]);}
			}
			break;
		case "setToRead":
			if (input.length == 2) {controller.setToRead(input[1]);}
			break;
		case "suggestRead":
			if (input.length == 1) {controller.suggestRead();}
			break;
		case "help":
			help();
			break;
...
public static void help() {
	...
	System.out.println("| Below is a list of valid commands. If the command \n"
			+ "| name is preceeded by a star (*), then it requires \n"
	...
	System.out.println("| suggestRead : 	Suggest a random unread book from your colleciton.");
	System.out.println('+' + "-".repeat(100));
